...
def createGraph(request):
    ...
    try:
        ...
        return render(request, 'admin/newgraph.html', {
                                'no_graph':"Kindly refresh the page , refill the data and the graph will be generated"})
...
def viewstore(request, id):
    ...
    if request.session.has_key('user'):
        ...
        listproduct=[]
        for i in prods:
            d=StockDetails.objects.get(productName=i.productname)
            a={
                'stockqty':d.quantity,
        ...
        return render(request, 'admin/storedetails.html', {'data': model, 'prod': listproduct})
    else:
        return redirect('user:signin')
...
def Create_Pdf(request, dt):
    if request.session.has_key('user'):
        ...
        order_id = 'SKBill_Id'+str(time_now)
...
def allbill(request):
    if 'user' in request.session:
        pdBill = SK_Bills.objects.all().distinct()
        
        blist=[]
        datadict={}
        for i in pdBill:
            if str(i.Bill_No) not in datadict.values():
                datadict={
                    'name':i.store_person,
                    'email':i.store_person,
                    'Billno':str(i.Bill_No),
                    'billdate':i.date_data
                }
            
                blist.append(datadict)
            print(blist)
        
        return render(request,'allbill.html',{'pdBill':blist})
    else:
        return redirect('user:signin')